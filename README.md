...
**v3.x** scoring compares **native n8n node types** (e.g. `n8n-nodes-base.webhook`, `n8n-nodes-base.openAi`). Older community package releases that submitted DNG mirror labels are **not** accepted by the cloud scorer (`client_version_unsupported`) until you upgrade to **≥ 3.0.0**.
...
2. Create a workflow using **standard n8n nodes** (Webhook, Set, OpenAI, IF, HTTP Request, LangChain Agent, …):
...
2. Confirm the install. **DNG Submit Level** (and the credential type) appear after reload.
